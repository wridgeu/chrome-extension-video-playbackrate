...
const initializeExtension = async (): Promise<void> => {
    let sliderComponent = document.getElementById(
        'sliderWebComponent'
    ) as UI5Slider
     ...
     */
    let videoElementsOnPage =
        retrievedResults.result as VideoElementIdentifier[]
...
const initializeSliderComponent = async (
    sliderComponent: UI5Slider
): Promise<void> => {
    let { latestSpeedAdjustment } = await chrome.storage.sync.get(
        'latestSpeedAdjustment'
    )
